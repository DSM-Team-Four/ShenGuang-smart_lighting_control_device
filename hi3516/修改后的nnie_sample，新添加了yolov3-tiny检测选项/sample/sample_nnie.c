    ...
    SAMPLE_SVP_NNIE_INPUT_DATA_INDEX_S *pstInputDataIdx)
{
    ...
    if (SVP_BLOB_TYPE_U8 <= pstNnieParam->astSegData[u32SegIdx].astSrc[u32NodeIdx].enType &&
        ...
        printf("u32VarSize is U8\n");
    } else {
        u32VarSize = sizeof(HI_U32);
        printf("u32VarSize is U32\n");
    }
    ...
    SAMPLE_SVP_NNIE_YOLOV3TINY_SOFTWARE_PARAM_S *pstSoftWareParam)
{
    ...
    pstSoftWareParam->u32OriImWidth = pstNnieParam->astSegData[0].astSrc[0].unShape.stWhc.u32Width;
    pstSoftWareParam->u32BboxNumEachGrid = 3;
    pstSoftWareParam->u32ClassNum = 8;
    pstSoftWareParam->au32GridNumHeight[0] = 13;
    pstSoftWareParam->au32GridNumHeight[1] = 26;
    pstSoftWareParam->au32GridNumWidth[0] = 13;
    pstSoftWareParam->au32GridNumWidth[1] = 26;
    ...
    HI_U32 u32BaseWidth,HI_U32 u32BaseHeight)
{
    ...
    // s32Ret = SAMPLE_SVP_NNIE_FillSrcData(&stNnieCfg, &s_stYolov3tinyNnieParam, &stInputDataIdx);
    // SAMPLE_SVP_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, YOLOV3TINYVO_FAIL_0, SAMPLE_SVP_ERR_LEVEL_ERROR,
    //     "Error,SAMPLE_SVP_NNIE_FillSrcData failed!\n");
...
// // 与Rfcn不同YOLOv3tiny是调用 SAMPLE_SVP_NNIE_Forward 进行解析
//     s32Ret = SAMPLE_SVP_NNIE_Forward(&s_stYolov3tinyNnieParam,&stInputDataIdx,&stProcSegIdx,HI_TRUE);
 
//     SAMPLE_SVP_CHECK_EXPR_RET(HI_SUCCESS != s32Ret,s32Ret,SAMPLE_SVP_ERR_LEVEL_ERROR,
//         "Error,SAMPLE_SVP_NNIE_Yolov3tiny_Proc failed!\n");
    /*draw result, this sample has 21 classes:
     class 0:background     class 1:plane           class 2:bicycle
     class 3:bird           class 4:boat            class 5:bottle
     class 6:bus            class 7:car             class 8:cat
  ...
  s32Ret = SAMPLE_SVP_NNIE_RoiToRect_Yolov3tiny(&(pstSwParam->stDstScore),
...
static HI_VOID* SAMPLE_SVP_NNIE_Yolov3tiny_ViToVo_thread(HI_VOID* pArgs)
{
    ...
    SAMPLE_SVP_NNIE_YOLOV3TINY_SOFTWARE_PARAM_S *pstSwParam;
    VIDEO_FRAME_INFO_S stBaseFrmInfo;
    VIDEO_FRAME_INFO_S stExtFrmInfo;
    ...
    pstParam = &s_stYolov3tinyNnieParam;
    pstSwParam = &s_stYolov3tinySoftwareParam;
    ...
    while (HI_FALSE == s_bNnieStopSignal)
    {
        ...
        s32Ret = SAMPLE_SVP_NNIE_Yolov3tiny_Proc_ViToVo(pstParam,pstSwParam, &stExtFrmInfo,
        stBaseFrmInfo.stVFrame.u32Width,stBaseFrmInfo.stVFrame.u32Height);
        SAMPLE_CHECK_EXPR_GOTO(HI_SUCCESS!=s32Ret, BASE_RELEASE,
            "Error(%#x),SAMPLE_SVP_NNIE_YOLOV3TINY_Proc failed!\n", s32Ret);
...
void SAMPLE_SVP_NNIE_Yolov3tiny_Vivo(void)
{
 //   HI_CHAR *pcSrcFile = "./data/nnie_image/rgb_planar/dog_bike_car_416x416.bgr";
    HI_CHAR *pcModelName = "./data/nnie_model/detection/yolo-tiny-people-10000.wk";
    SAMPLE_SVP_NNIE_CFG_S   stNnieCfg = {0};
    SIZE_S stSize;
    PIC_SIZE_E enSize = PIC_CIF;
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    /******************************************
     step 1: start vi vpss vo
     ******************************************/
    s_stYolov3tinySwitch.bVenc = HI_FALSE;
    s_stYolov3tinySwitch.bVo   = HI_TRUE;
    s32Ret = SAMPLE_COMM_IVE_StartViVpssVencVo(&s_stViConfig,&s_stYolov3tinySwitch,&enSize);
    SAMPLE_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret, YOLOV3TINY_FAIL_1,
    ...
    pthread_create(&s_hNnieThread, 0, SAMPLE_SVP_NNIE_Yolov3tiny_ViToVo_thread, NULL);
...
YOLOV3TINY_FAIL_0:
    SAMPLE_COMM_IVE_StopViVpssVencVo(&s_stViConfig,&s_stYolov3tinySwitch);
...
void SAMPLE_SVP_NNIE_Yolov3tiny_Vivo_HandleSig(void)
{
    ...
    memset(&s_stYolov3tinySoftwareParam,0,sizeof(SAMPLE_SVP_NNIE_YOLOV3_SOFTWARE_PARAM_S));
