...
static HI_VOID Yolo2HandClassifyAiProcess(VIDEO_FRAME_INFO_S frm, VO_LAYER voLayer, VO_CHN voChn)
{
    int ret;
...
static HI_VOID Yolo3tinyHandClassifyAiProcess(VIDEO_FRAME_INFO_S frm, VO_LAYER voLayer, VO_CHN voChn)
{
    int ret;
    if (GetCfgBool("hand_classify_switch:support_hand_classify", true)) {
        if (g_workPlug.model == 0) {
            ret = Yolo3tinyHandDetectResnetClassifyLoad(&g_workPlug.model);
            if (ret < 0) {
                g_workPlug.model = 0;
                SAMPLE_CHECK_EXPR_GOTO(ret < 0, HAND_RELEASE,
                    "load hand classify model err, ret=%#x\n", ret);
        ...
        VIDEO_FRAME_INFO_S resizeFrm;
        ret = MppFrmResize(&frm, &resizeFrm, HAND_FRM_WIDTH, HAND_FRM_HEIGHT);
        ret = Yolo3tinyHandDetectResnetClassifyCal(g_workPlug.model, &resizeFrm, &frm);
        SAMPLE_CHECK_EXPR_GOTO(ret < 0, HAND_RELEASE,
            "hand classify plug cal FAIL, ret=%#x\n", ret);
...
static HI_VOID* GetVpssChnFrameHandClassify(HI_VOID* arg)
{
    ...
    while (HI_FALSE == g_bAiProcessStopSignal) {
        ...
        Yolo3tinyHandClassifyAiProcess(frm, voLayer, voChn);
        //Hereway
        //Yolo2HandClassifyAiProcess(frm, voLayer, voChn);
    }
...
static HI_S32 PauseDoUnloadHandClassifyModel(HI_VOID)
{
    ...
    if (GetCfgBool("hand_classify_switch:support_hand_classify", true)) {
        ...
        //Here to unload
        s32Ret = Yolo3tinyHandDetectResnetClassifyUnload(g_workPlug.model);
...
/*
 * 将sensor采集到数据显示到液晶屏上，同时创建线程运行垃圾分类推理计�??
 * 视频输入->视频处理子系�??->视频输出->显示�??
 *
